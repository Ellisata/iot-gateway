...
func TestGetProtocolDataTypes(t *testing.T) {
	...
	vo, err := svc.GetProtocolDataTypes(ctx, "ModBus.Net.TCP", "")
	if err != nil {
		t.Fatalf("query by name: %v", err)
	}
	if vo.Protocol != "ModBus.Net.TCP" {
		t.Errorf("protocol = %q, want ModBus.Net.TCP", vo.Protocol)
	}
	...
	if vo2.Protocol != "ModBus.Net.TCP" {
		t.Errorf("protocol (by id) = %q, want ModBus.Net.TCP", vo2.Protocol)
	}
