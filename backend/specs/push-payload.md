...
- **DL/T 645 是例外**：数据的字节数、小数位与符号由**数据标识 DI** 决定，不由 `dataType` 决定。
  故 `float` 输出的是已按 DI 小数位定标的工程值，而 `bcd`/`lbcd`/`uint`/`int` 输出**未定标**的
  原始整数；同一 `dataType`（如 `uint`）在不同 DI 下量纲不同，消费端须按点位语义解释。
...
| DL/T 645 | `datetime`（日期 DI） | `YYYY-MM-DD` | `2026-08-19` |
| DL/T 645 | `datetime`（时间 DI） | `HH:MM:SS` | `16:39:47` |
...
- DL/T 645 的日期/时间为 BCD 编码，同样**不含时区**；年份只传两位，驱动统一按 2000 年后解释。
  字段布局不合规范时驱动置 `quality=0` 并留空值，**不会**猜测出一个错误日期。
