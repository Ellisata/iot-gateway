...
func TestMigrationFormFieldsMatchConfigStruct(t *testing.T) {
	...
	if err := db.Table("iot_protocol").
		...
		Where("name LIKE ?", "Mitsubishi%").
		Scan(&rows).Error; err != nil {
		t.Fatalf("查询 iot_protocol 失败: %v", err)
	}
	if len(rows) != 2 {
		t.Fatalf("迁移后 Mitsubishi 协议数 = %d, want 2", len(rows))
	}

	valid := map[string]bool{}
	rt := reflect.TypeOf(MCConfig{})
	...
	for _, r := range rows {
		...
		for _, f := range form.Rule {
			...
			if !valid[f.Field] {
				t.Errorf("%s 的表单字段 %q 在 MCConfig 中没有对应的 json tag（填了也会被驱动忽略）",
					r.Name, f.Field)
			}
	...
	for _, proto := range []string{ProtocolMCTCP, ProtocolMCSerial} {
		if !seen[proto] {
