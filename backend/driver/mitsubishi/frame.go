...
// buildMCBody 构建请求体（命令 + 子命令 + 设备码 + 首地址号 + 点数，11 字节）。
// TCP（3E 帧）与串口（4C Format5）共用此应用层编码。
// 3E/SLMP 二进制帧所有多字节字段均小端（低位在前）：命令 0x0401 线上为 01 04。
//
//	命令(2 LE) | 子命令(2 LE) | 设备码(2 LE) | 首地址号(3 LE) | 点数(2 LE)
...
// 与帧内其余多字节字段一致，均为小端：长度 0x000D → 0D 00，定时器 0x0010 → 10 00。
const (
	reqDataLenRead = 0x000D // 请求数据长度：监视定时器(2) + 请求体(11)
	monitorTimer   = 0x0010 // 监视定时器：16 × 250ms = 4s
)

// buildReadFrame 构建 3E 帧 TCP 读请求（22 字节，无 STX/ETX/和校验）。
//
