...
// s7Scope S7 类型注册表作用域缓存。
// protocolName 为常量，作用域前缀固定；每点位直接复用该作用域，避免反复
// 构造 ProtocolScope 与拼接 "{protocol}." 前缀。
var s7Scope = driver.GetTypeRegistry().ForProtocol(protocolName)

var (
	s7LookupOnce sync.Once
	s7Types      map[string]*driver.DataType
)

// s7Lookup 按裸名查 DataType：优先命中静态缓存（零分配、无锁），
// 未命中回退注册表查询（兼容大小写变体，保持原有不区分大小写语义）。
// 类型注册表在 init() 后不再变化（dataTypeMap 仅查询不注册），
// 按裸名缓存可消除每点位对全局注册表 RWMutex 的查询与锁竞争。
func s7Lookup(name string) (*driver.DataType, bool) {
	s7LookupOnce.Do(func() {
	...
	if dt, ok := s7Types[name]; ok {
		return dt, true
	}
	return s7Scope.Get(name)
}
